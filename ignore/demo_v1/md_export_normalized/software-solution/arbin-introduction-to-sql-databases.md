...
|-----------|-------------|
| Write/Read | Master Database Server has higher data persistency. |
| Performance | Higher performance and synchronization between Master and Replicate Servers. |
| Data Analysis | High-speed data analysis, query, and export from Replicate Server without impacting current Master Database Performance. |
...
| Network Requirement | Requires high-speed network to ensure synchronization bandwidth between Master and Replicate Server. |
...
All the remote databases will be hosted in AWS, GCP, Azure, and other hosting servers at data centers to keep your data backed up remotely. This option allows you to keep a copy of your data on cloud servers, but not only cloud database services.

### Advantages

| Advantage | Description |
|-----------|-------------|
| Remote Backup | Having a remote backup policy to prevent disastrous incidents at local office will lose all valuable test data. |
...
| Disadvantage | Description |
|--------------|-------------|
| Network Latency | Due to network latency between office and cloud service provider, there is a potential risk of data inconsistency between local and off-site servers. |
...
*All units in pps (points per second). Arbin data point takes 10 bytes in size, which contains two floats (1 byte per float number) and two unsigned chars (4 bytes per char) which adds up to 10 bytes.*
