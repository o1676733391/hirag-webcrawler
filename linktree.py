...
def is_allowed_language_url(full_url, domain, language_mode='english', allowed_locales=None):
    if allowed_locales is None:
        allowed_locales = {'en', 'en-us', 'en-gb'}
...
def normalize_internal_page_url(current_url, href, domain, language_mode='english', allowed_locales=None):
    ...
    if not is_allowed_language_url(clean_url, domain, language_mode=language_mode, allowed_locales=allowed_locales):
        return None
...
def collect_unique_links(link_tree, categories=None):
    unique = []
    seen = set()
    ...
    if categories:
        for links in categories.values():
            for link in links:
                if link not in seen:
                    seen.add(link)
                    unique.append(link)
...
def export_unique_links_to_txt(unique_links, output_file):
    lines = ["=== UNIQUE LINKS ===", ""] + [f"- {link}" for link in unique_links]
    Path(output_file).write_text('\n'.join(lines), encoding='utf-8')
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Crawl website link tree and export categorized txt files."
    )
    parser.add_argument(
        '--url',
        required=True,
        help='Target website root URL (required).',
    )
    parser.add_argument(
        '--max-depth',
        type=int,
        default=1,
        help='Maximum crawl depth from root URL.',
    )
    parser.add_argument(
        '--language-mode',
        choices=['english', 'all'],
        default='english',
        help='Filter links by language prefix (english) or keep all languages (all).',
    )
    parser.add_argument(
        '--allowed-locales',
        default='en,en-us,en-gb',
        help='Comma-separated locale prefixes used when --language-mode english.',
    )
    parser.add_argument(
        '--timeout',
        type=int,
    ...
    parser.add_argument(
        '--output-tree',
        default='link_tree.txt',
        help='Output txt path for link tree export.',
    )
    parser.add_argument(
        '--output-categories-dir',
        default='categories_export',
        help='Output directory for category txt exports.',
    )
    parser.add_argument(
        '--output-unique-links',
        default='unique_links.txt',
        help='Output txt path for globally unique links.',
    )
...
def main():
    args = parse_args()
    allowed_locales = parse_allowed_locales(args.allowed_locales)

    link_structure = crawl_link_tree(
        args.url,
    ...
    dropdown_categories = extract_dropdown_categories(
        args.url,
        language_mode=args.language_mode,
        allowed_locales=allowed_locales,
        timeout=max(1, args.timeout),
    ...
    export_unique_links_to_txt(unique_links, args.output_unique_links)
    export_link_tree_to_txt(link_structure, args.output_tree)
    export_categories_to_txt(category_structure, args.output_categories_dir)
